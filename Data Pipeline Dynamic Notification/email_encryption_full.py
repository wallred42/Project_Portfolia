# Virtru app ID for : 506388f6-2156-4b01-baa3-805e661cbb9f
import smtplib
...
def email_encryption(smtp_to_address, file_path_plain, subject):
    # Variables 
    # SMTP Variables
    ...
    # File Variables
    file_name_tdf = file_path_plain + ".tdf3.html" # file that you want encrypted + tdf3.html at the end
...
def get_inbox():
    ...
    for num in search_data[0].split():
        ...
        if email_message['from'] == 'Mail Delivery Subsystem <mailer-daemon@googlemail.com>':
            ...
            to_bounced_email = ''  # change this to whoever will fix bounced and errors in email 
            msg['From'] = '' # change to same as smtp_from_address 
            ...
            s.login('', password)
            text = msg.as_string()
            s.sendmail('', to_bounced_email, text)
            s.quit()
